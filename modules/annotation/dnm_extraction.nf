...
process DNM_EXTRACTION {
  ...
  */
  ...
  slivar expr \\
    ...
    --trio "denovo_x_male:(variant.CHROM == 'chrX' && kid.sex == 'male' && (variant.POS < ${par1_start} || (variant.POS > ${par1_end} && variant.POS < ${par2_start}) || variant.POS > ${par2_end}) && kid.hom_alt && kid.DP >= ${min_dp} && kid.GQ >= ${min_gq} && mom.hom_ref && mom.AB <= 0.02 && mom.DP >= ${min_dp} && mom.GQ >= ${min_gq})" \\
    --trio "denovo_x_female_het:(variant.CHROM == 'chrX' && kid.sex == 'female' && (variant.POS < ${par1_start} || (variant.POS > ${par1_end} && variant.POS < ${par2_start}) || variant.POS > ${par2_end}) && kid.het && kid.DP >= ${min_dp} && kid.GQ >= ${min_gq} && kid.AB >= ${min_vaf} && mom.hom_ref && mom.AB <= 0.02 && mom.DP >= ${min_dp} && mom.GQ >= ${min_gq} && dad.hom_ref && dad.AB <= 0.02 && dad.DP >= ${min_dp} && dad.GQ >= ${min_gq})" \\
    --trio "denovo_x_female_hom:(variant.CHROM == 'chrX' && kid.sex == 'female' && (variant.POS < ${par1_start} || (variant.POS > ${par1_end} && variant.POS < ${par2_start}) || variant.POS > ${par2_end}) && kid.hom_alt && kid.AB >= 0.98 && kid.DP >= ${min_dp} && kid.GQ >= ${min_gq} && mom.hom_ref && mom.AB <= 0.02 && mom.DP >= ${min_dp} && mom.GQ >= ${min_gq} && dad.hom_ref && dad.AB <= 0.02 && dad.DP >= ${min_dp} && dad.GQ >= ${min_gq})" \\
    ...
    --trio "denovo_y_male:(variant.CHROM == 'chrY' && kid.sex == 'male' && kid.hom_alt && kid.DP >= ${min_dp} && kid.GQ >= ${min_gq} && dad.hom_ref && dad.AB <= 0.02 && dad.DP >= ${min_dp} && dad.GQ >= ${min_gq})"
